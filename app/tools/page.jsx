...
export default function ToolsPage(){ const tools=[
  ...
  {slug:'windows-gamemode',name:'Input/Display Optimizer',desc:'Reduce input lag and auto-calibrate display.'},
]; return (<div className="max-w-6xl mx-auto px-4 py-8"><h1 className="text-2xl font-bold mb-6">OptiPlay Tools</h1>
  <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">{tools.map(t=>(<Link key={t.slug} href={`/tools/${t.slug}`} className="block bg-[#0b0b10] border border-white/10 rounded p-4 hover:border-optiPurple-500 transition-transform duration-200 hover:-translate-y-0.5 hover:shadow-[0_8px_24px_rgba(0,0,0,0.35)] animate-fade-in"><div className="font-semibold">{t.name}</div><div className="text-sm text-slate-300 mt-1">{t.desc}</div></Link>))}</div></div>) }

  ...
  return (
    <div className={`relative ${className}`}>
      ...
      {open && suggestions.length ? (
        <div className="fixed left-1/2 -translate-x-1/2 z-30 mt-2 w-[40rem] rounded-lg border border-white/10 bg-neutral-950/95 shadow-lg shadow-black/30">
          <ul className="divide-y divide-white/5 text-sm text-gray-200">
            {suggestions.map((item) => (
              <li key={item.href}>
                <Link
                  href={item.href}
                  className="block px-4 py-3 hover:bg-purple-600/20"
                  onMouseDown={(event) => event.preventDefault()}
                  onClick={() => setOpen(false)}
                >
                  <div className="flex items-center justify-between text-[11px] uppercase tracking-wide text-purple-300 mb-1">

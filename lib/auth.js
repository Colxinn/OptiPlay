...
function sanitizeImageForToken(image) {
  ...
  // Allow data URLs (base64 images) - they're stored in the database
  // Allow regular URLs
  if (image.length > 50000) return null; // Reasonable limit for base64 images
  return image;
}

// Build providers array dynamically to avoid build-time errors
function getProviders() {
  const providers = [];

  // Only add EmailProvider if we have proper config OR in development
  const hasEmailConfig = process.env.AUTH_EMAIL_SERVER || process.env.NODE_ENV === "development";
    ...
    providers.push(
      EmailProvider({
        ...
        async sendVerificationRequest({ identifier, url, provider }) {
          // In development without email server, just log the magic link
          if (process.env.NODE_ENV === "development" && !process.env.AUTH_EMAIL_SERVER) {
            console.log("\n" + "=".repeat(80));
            console.log("🔐 MAGIC LINK FOR LOCAL TESTING");
            console.log("=".repeat(80));
            console.log(`Email: ${identifier}`);
            console.log(`Login URL: ${url}`);
            console.log("=".repeat(80) + "\n");
            console.log("💡 Copy the URL above and paste it in your browser to sign in");
            console.log("   Or check your terminal for MailHog/MailCatcher if running\n");
    ...
    async jwt({ token, user }) {
      ...
      try {
        ...
        if (dbUser) {
          ...
          token.isOG = dbUser.isOG ?? token.isOG ?? false;
...
// NextAuth 4.x returns a handler function for App Router
const handler = NextAuth(authOptions);

// Export the handler for use in route.js
export { handler as nextAuthHandler };

// For getting the session in server components
// We need to use getServerSession or implement auth
// For now, we'll use a simple wrapper
export const auth = async (req, res) => {
  // This is a placeholder - in NextAuth 4, you typically use the handler directly
  // or getServerSession for server-side session retrieval
  return null;
};

export const signIn = async () => {
  throw new Error('Use next-auth/react signIn in client components');
};

export const signOut = async () => {
  throw new Error('Use next-auth/react signOut in client components');
};

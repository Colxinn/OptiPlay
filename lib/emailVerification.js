...
// Rate limiting for email sends to prevent quota exhaustion
const emailSendCache = new Map();
const EMAIL_SEND_LIMIT = 5; // Max 5 emails per email address per hour
const EMAIL_SEND_WINDOW = 60 * 60 * 1000; // 1 hour

function canSendEmail(email) {
  const now = Date.now();
  const key = email.toLowerCase();
  const record = emailSendCache.get(key) || { count: 0, resetAt: now + EMAIL_SEND_WINDOW };
  
  // Reset if window expired
  if (now > record.resetAt) {
    emailSendCache.set(key, { count: 1, resetAt: now + EMAIL_SEND_WINDOW });
    return true;
  }
  
  // Check limit
  if (record.count >= EMAIL_SEND_LIMIT) {
    return false;
  }
  
  // Increment count
  record.count++;
  emailSendCache.set(key, record);
  return true;
}
...
export async function sendVerificationEmail(email, code, username) {
  // PROTECTION: Prevent email quota exhaustion from spam attacks
  if (!canSendEmail(email)) {
    console.warn(`⚠️ Email send rate limit hit for ${email}`);
  ...
  const authEmailFrom = process.env.AUTH_EMAIL_FROM || 'OptiPlay <noreply@trial-k68zxl2jy7pg9yjr.mlsender.net>';
...
© ${new Date().getFullYear()} OptiPlay. All rights reserved.
    ...
    if (error.response?.statusCode === 429 || error.response?.body?.message?.includes('quota')) {
      throw new Error('Email service quota reached. Please try again later or contact support.');
    }
    
    // Check for domain verification error
    if (error.response?.statusCode === 401 || error.message?.includes('domain')) {

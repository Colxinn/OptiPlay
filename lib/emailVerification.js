...
export async function sendVerificationEmail(email, code, username) {
  ...
  const fromEmail = process.env.MAILERSEND_FROM_EMAIL || 'noreply@optiplay.gg';
  const fromName = process.env.MAILERSEND_FROM_NAME || 'OptiPlay';

  if (!mailerSendApiKey) {
    console.error('❌ MAILERSEND_API_KEY not set in environment variables');
